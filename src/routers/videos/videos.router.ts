...
videosRouter.post(
  "/",
  (req: Request<unknown, unknown, VideoCreateModel>, res: Response<VideoResponseModel>) => {
...
videosRouter.put(
  "/:id",
  (req: Request<{ id: string }, unknown, VideoUpdateModel>, res: Response<VideoResponseModel>) => {
    const requestId = parseInt(req.params.id);
    if (isNaN(requestId)) {
      res.sendStatus(HttpStatus.NotFound);
      return;
